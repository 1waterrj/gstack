...
name: support
preamble-tier: 2
description: App support agent. (gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
Triage an incoming support ticket, investigate the root
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"support","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"support","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"support","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
### Provisioning intake when none exists

If the project has no way for customers to submit tickets yet, don't prescribe
one — the right pipeline is specific to what's being built. Offer to provision
it (or hand off to the engineering flow), and let the choice be the user's:
anything that produces tickets in the canonical contract above works. Common
starting points: GitHub Issues (a `support_fetch`/`support_list` wired to
`gh issue view`/`gh issue list`), an in-app submission form posting to a small
endpoint or table, an inbound-email parser, or a shared CSV. Whatever you build,
make it emit the contract (or be readable by `support_fetch`/`support_list`) so
the rest of this skill consumes it with no glue, then persist the commands to
CLAUDE.md.
